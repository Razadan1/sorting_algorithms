 ...
 * of integers in the ascending order
 *
...
void insertion_sort_list(listint_t **list)
{
	...
	while (node != NULL)
	{
		...
		next = node->next;
		tmp_next = tmp->next;
		/* Make NULL for the last node */
		tmp_prev = tmp->prev;
		/* Move d current node bkward while its prev node value is greater*/
		while (tmp->prev != NULL && tmp_prev->n > tmp->n)
		{
			...
			/*Adjust temp pointers for d next iteration*/
			tmp->next = tmp_prev;
			tmp_prev = tmp->prev;
			tmp_next = tmp->next;
